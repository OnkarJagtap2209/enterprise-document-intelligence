...
def build_grounded_prompt(question: str, context: Sequence[ContextItem]) -> str:
    ...
    return "SYSTEM:\nAnswer only from supplied evidence. Do not invent facts or citations. State when evidence is insufficient. Return JSON with keys answer and source_ids; source_ids must contain only supplied SOURCE IDs.\n\nUSER QUESTION:\n" + question.strip() + "\n\nSUPPLIED EVIDENCE:\n" + evidence
